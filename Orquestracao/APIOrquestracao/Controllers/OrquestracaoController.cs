...
public class OrquestracaoController : ControllerBase
{
    ...
    public async Task<ResultadoOrquestracao> Get()
    {
        ...
        var urlApiMySQL = _configuration["APIContagemMySQL"]!;
        resultado.ContagemMySQL =
            await _contagemClient.ObterContagemAsync(urlApiMySQL);
        _logger.LogInformation($"Valor contagem MySQL: {resultado.ContagemMySQL!.ValorAtual}");
